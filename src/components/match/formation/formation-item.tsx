...
export default function FormationItem({ formation, isLeft }: FormationItemProps) {
  ...
  return (
    <div
      className={`relative flex h-[300px] w-full flex-row rounded-lg bg-[#247847] sm:h-full sm:rounded-none md:w-1/2 ${isLeft ? 'sm:rounded-l-lg sm:pr-8' : 'sm:rounded-r-lg sm:pl-8'}`}
    >
      {/*골대 ui*/}
      <span
