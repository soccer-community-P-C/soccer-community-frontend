...
export default function Pagination({ totalPages }: PaginationProps) {
  ...
  const pageParam = searchParams.get('page') ?? String(MIN_PAGE);

  let currentPage = Math.min(Number(pageParam), totalPages);
  currentPage = Math.max(MIN_PAGE, currentPage);
  ...
  return (
    <nav aria-label="페이지 네비게이션" className="my-2">
        ...
        {currentPage > MIN_PAGE ? (
          <li>
            ...
            dark:hover:bg-gray-700 dark:hover:text-white sm:px-4"
              href={`${pathname}?page=${Math.max(MIN_PAGE, currentPage - pageSize)}`}
            >
